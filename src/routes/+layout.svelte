<script>
	...
	import { getSessionUser, userSignOut, demoAuth } from '$lib/apis/auths';
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} • eagleGPT`, {
								body: content,
	...
	const channelEventHandler = async (event) => {
		...
		if ((!channel || isFocused) && event?.user?.id !== $user?.id) {
			...
			if (type === 'message') {
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${data?.user?.name} (#${event?.channel?.name}) • eagleGPT`, {
							body: data?.content,
